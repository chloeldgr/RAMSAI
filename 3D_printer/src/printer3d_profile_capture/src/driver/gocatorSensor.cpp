                    ...
                    for (k = 0; k < GoResampledProfileMsg_Count(profileMsg); ++k)
                    {
                        ...
                        for (arrayIndex = 0; arrayIndex < GoResampledProfileMsg_Width(profileMsg); ++arrayIndex)
                        {
                            ...
                            if (data[arrayIndex] != INVALID_RANGE_16BIT )
                            {
                                ...
                                _p_cloud.points.at(k*_p_cloud.width+arrayIndex).x = XOffset + XResolution * arrayIndex;
                                _p_cloud.points.at(k*_p_cloud.width+arrayIndex).z = ZOffset + ZResolution * data[arrayIndex];
                                validPointCount++;
                            }
                            else
                            {
                    ...
                    for (k = 0; k < GoProfileMsg_Count(profileMsg); ++k)
                    {
                        ...
                        for (arrayIndex = 0; arrayIndex < GoProfileMsg_Width(profileMsg); ++arrayIndex)
                        {
                            if (data[arrayIndex].x != INVALID_RANGE_16BIT)
                            {
                                ...
                                _p_cloud.points.at(k*_p_cloud.width+arrayIndex).x = XOffset + XResolution * data[arrayIndex].x;
                                _p_cloud.points.at(k*_p_cloud.width+arrayIndex).z = ZOffset + ZResolution * data[arrayIndex].y;
                                validPointCount++;
                            }
                            else
                            {
...
void GocatorSensor::Device::getTemperature(double & _internal_temp, double & _projector_temp, double & _laser_temp) const
{
    //local variables
    GoDataSet health_data = kNULL;
    GoHealthMsg health_msg =kNULL;
