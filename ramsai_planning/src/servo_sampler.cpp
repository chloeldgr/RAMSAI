...
moveit_msgs::action::LocalPlanner::Feedback
ServoSampler::getLocalTrajectory(
  ...
  robot_trajectory::RobotTrajectory & local_trajectory)
{
  ...
  if (pass_through_) {
    ...
    local_trajectory.append(
      *reference_trajectory_, 0.0, 0,
      reference_trajectory_->getWayPointCount());
  } else {
    // Get next desired robot state
    moveit::core::RobotState next_desired_goal_state = reference_trajectory_->getWayPoint(
    ...
    for (size_t i = next_waypoint_index_; i < reference_trajectory_->getWayPointCount(); ++i) {
      local_trajectory.addSuffixWayPoint(
        reference_trajectory_->getWayPoint(i),
